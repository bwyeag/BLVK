...
#define UTF_CPP_STATIC_ASSERT(condition) \
    static_assert(condition, "UTFCPP static assert");
#else  // C++ 98/03
...
struct StaticAssert {
    static void assert() { int static_assert_impl[(Condition ? 1 : -1)]; }
};
template <>
...
const utfchar16_t LEAD_OFFSET =
    0xd7c0u;  // LEAD_SURROGATE_MIN - (0x10000 >> 10)
...
utf_error validate_next16(word_iterator& it,
                          word_iterator end,
                          utfchar32_t& code_point) {
...
// so we assume it's utfchar8_t; that can cause a conversion warning if we are
// wrong.
template <typename octet_iterator>
octet_iterator append(utfchar32_t cp, octet_iterator result) {
